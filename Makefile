...
$(PROGRAM): $(SRC)
ifeq ($(GOOS),linux)
	GOPATH=$(RUNTIME_GOPATH) go build -ldflags "-X gslap.version=$(VERSION)" -a -tags netgo -installsuffix netgo -o $(PROGRAM)
else
	GOPATH=$(RUNTIME_GOPATH) go build -ldflags "-X gslap.version=$(VERSION)" -o $(PROGRAM)
endif
...
rpm/docker: clean go-get
	cd ../ && tar zcf gslap.tar.gz src
	mv ../gslap.tar.gz /root/rpmbuild/SOURCES/
	cp gslap.spec /root/rpmbuild/SPECS/
	rpmbuild -ba /root/rpmbuild/SPECS/gslap.spec
	mv /root/rpmbuild/RPMS/x86_64/gslap-*.rpm pkg/
	mv /root/rpmbuild/SRPMS/gslap-*.src.rpm pkg/
